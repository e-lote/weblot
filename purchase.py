	...
	for line in self.browse(cr, uid, ids, context=context):
		...
		if supplier_id:
			...
			if supplier.carton_quantity > 0:
				res[line.id] = (math.ceil(line.product_qty / supplier.carton_quantity)) * supplier.carton_volume
			else:
				res[line.id] = 0
	...
	for line in self.browse(cr, uid, ids, context=context):
		...
		if supplier_id:
			...
			if supplier.service_fee > 0:
				res[line.id] = line.product_qty * (supplier.supplier_price + supplier.developing_cost + supplier.royalties + \
